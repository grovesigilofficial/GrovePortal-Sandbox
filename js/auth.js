...
document.getElementById('next-2').onclick = ()=>{
  ...
  if(!name){ 
    document.getElementById('name-error').innerText="Full name required"; 
    return; 
  }
...
document.getElementById('next-3').onclick = async ()=>{
  ...
  // Temporary: skip uniqueness check until table exists
  currentStep++; 
  showStep(currentStep);
};
...
document.getElementById('next-4').onclick = ()=>{
  ...
  if(pass.length<6){ 
    document.getElementById('password-error').innerText="Password too short"; 
    return; 
  }
  if(pass!==confirm){ 
...
document.getElementById('submit-signup').onclick = async ()=>{
  ...
  if(!dob){ 
    document.getElementById('dob-error').innerText="DOB required"; 
    return; 
  }
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      data: { full_name: fullName, username: username, dob }
    }
  });

  if(error){
    document.getElementById('dob-error').innerText = error.message; 
    return; 
  }
